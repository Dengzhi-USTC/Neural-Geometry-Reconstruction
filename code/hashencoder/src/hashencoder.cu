...
__global__ void kernel_grid(
    ...
    for (uint32_t dim = 0; dim < D; ++dim) {
        pos_fract(positions_in[i + dim * B], &pos[dim], &pos_grid[dim], scale, smoothstep);
    }	
...
__global__ void kernel_grid_grad_inputs_pre(
    ...
    #pragma unroll
    for (uint32_t dim = 0; dim < D; ++dim) {
        pos_fract(positions_in[i + dim * B], &pos[dim], &pos_derivative[dim], &pos_grid[dim], scale, smoothstep, smoothstep_derivative);
    }	
	auto grid_val = [&](const uint32_t local_pos[D]) {
		uint32_t index = grid_index<D, C>(0, hashmap_size, grid_resolution, local_pos);
		return *(vector_t<scalar_t, C>*)&grid[index];
...
__global__ void kernel_grid_backward(
	...
	const uint32_t i = ((blockIdx.x * blockDim.x + threadIdx.x) * N_C) / C;
	if (i >= B) return;

	const uint32_t level = blockIdx.y ; // <- the level is the same for all threads.
	const uint32_t feature = (blockIdx.x * blockDim.x + threadIdx.x) * N_C - i * C;
	grid_gradient += offsets[level] * C;
	const uint32_t hashmap_size = offsets[level + 1] - offsets[level];

	const float scale = exp2f(level * S) * H - 1.0f;
	const uint32_t grid_resolution = ((uint32_t)ceil(scale) + 1);
	auto add_grid_gradient = [&](const uint32_t local_pos[D], const vector_t<scalar_t, N_C>& grad, const float weight) {
		...
		if (N_C > 1 && std::is_same<scalar_t, __half>::value) {
			for (uint32_t f = 0; f < N_C; f += 2) {
...
__global__ void kernel_grid_backward_grad_out_backward(
    ...
    #pragma unroll
    for (uint32_t dim = 0; dim < D; ++dim) {
        pos_fract(positions_in[dim], &pos[dim], &pos_grid[dim], scale, smoothstep);
    }	
	uint32_t pos_neighs[24] = {0};
	float weights_neighs[8] = {0};
	generate_weights_pos(pos, pos_grid, weights_neighs, pos_neighs);
    ...
    for (uint32_t idx = 0; idx < (1 << D); idx++) {
        ...
        for (uint32_t c = 0; c < N_C; c++) {
            grad_grad_out_grid[c] += w *grad_grad_grid[index + c];
        }
    }    
}
...
__global__ void kernel_input_backward_grid_backward(
    ...
    #pragma unroll
    for (uint32_t dim = 0; dim < D; ++dim) {
        pos_fract(positions_in[dim], &pos[dim], &pos_derivative[dim], &pos_grid[dim], scale, smoothstep, smoothstep_derivative);
    }
    // interpolate
...
)
{
    ...
    #pragma unroll
    for (uint32_t dim = 0; dim < D; ++dim) {
        pos_fract(positions_in[dim], &pos[dim], &pos_derivative[dim], &pos_derivative2[dim], &pos_grid[dim], scale, smoothstep, smoothstep_derivative, smoothstep_derivative_second);
    }
	...
	uint32_t pos_grid_local[D];
    float grad_grid_grad_input_tp[D] = {0};
	#pragma unroll
	for(uint32_t idx = 0; idx < (1<<D); ++idx)
	{
        pos_grid_local[0] = pos_neighs_dy_dx[idx*3];
		...
		for (uint32_t ch = 0; ch < C; ch++) {
            ...
            grad_grid_grad_input_tp[gd] += (w *float(grad_out[ch]*grad_grad_grid[index + ch]));
		}
	}
	}
    #pragma unroll
    for (int d= 0; d <D; d++)
    {
        atomicAdd(&grad_grid_grad_input[d], grad_grid_grad_input_tp[d]);
    }
	#pragma unroll
	for (uint32_t ch = 0; ch < C; ++ch) {
...
__global__ void kernel_input_backward_input_backward_pre(
    ...
    for (uint32_t dim = 0; dim < D; ++dim) {
        ...
        const uint32_t ch_start = (blockIdx.x * blockDim.x + threadIdx.x) * N_C - b * C;
        ...
        for (uint32_t idx = 0; idx < 1 << D; ++idx)
        {
            ...
            for (uint32_t gd1 = 0; gd1 < D; gd1++)
            {
                ...
                for (uint32_t gd2 = 0; gd2 < D; gd2++)
                {
                    ...
                    {
                        dy2_dx[(gd1*D +gd2)* C + ch + ch_start]+= w*(grid[index+ch + ch_start]);
                    }
                }
            }
        }
...
void kernel_grid_backward_backward_wrapper(const scalar_t * grad_out, const scalar_t * inputs,const scalar_t * grad_grad_inputs, const scalar_t * grid, const scalar_t * grad_grad_grid, const int * offsets, scalar_t * grad2_grid, scalar_t * grad2_input, scalar_t * dy2_dx, scalar_t * grad_grad_out, const uint32_t B, const uint32_t C, const uint32_t L, const float S, const uint32_t H)
{
    ...
    // const uint32_t N_C = std::min(2u, C); // N_C
    uint32_t N_C = 1;
    // printf("this is the Nc %d", N_C);
    ...
    case 1: 
        ...
        kernel_backward_part1<scalar_t, D, 1><<<blocks_hashgrid1, N_THREAD1>>>(inputs, grid, grad_grad_grid, grad_out, grad_grad_inputs, offsets, B, L, S, H, grad2_input, grad_grad_out);
        kernel_input_backward_input_backward_pre<scalar_t, D, 1, 1><<<blocks_hashgrid, N_THREAD>>>(inputs, grid, offsets, dy2_dx, B, L, S, H);
        // // 
    ...
    break;
    case 2: 
        kernel_input_backward_grid_backward<scalar_t, D, 2, 1><<<blocks_hashgrid, N_THREAD>>>(grad_out, inputs, grad_grad_inputs, offsets, grad2_grid, B, L, S, H);
        ...
        kernel_input_backward_input_backward_pre<scalar_t, D, 2, 1><<<blocks_hashgrid, N_THREAD>>>(inputs, grid, offsets, dy2_dx, B, L, S, H);
        // // 
        ...
        // // N_C grid---> grad_grad_out
        kernel_grid_backward_grad_out_backward<scalar_t, D, 2, 1><<<blocks_hashgrid, N_THREAD>>>(inputs, grad_grad_grid, offsets, grad_grad_out, B, L, S, H);
        break;
    case 4: 
                // grid
        // N_C input ---> grid
        kernel_input_backward_grid_backward<scalar_t, D, 4, 1><<<blocks_hashgrid1, N_THREAD1>>>(grad_out, inputs, grad_grad_inputs, offsets, grad2_grid, B, L, S, H);
        kernel_backward_part1<scalar_t, D, 4><<<blocks_hashgrid1, N_THREAD1>>>(inputs, grid, grad_grad_grid, grad_out, grad_grad_inputs, offsets, B, L, S, H, grad2_input, grad_grad_out);
        kernel_input_backward_input_backward_pre<scalar_t, D, 4, 1><<<blocks_hashgrid, N_THREAD>>>(inputs, grid, offsets, dy2_dx, B, L, S, H);
        // // // 
        ...
        // // // N_C grid---> grad_grad_out
        kernel_grid_backward_grad_out_backward<scalar_t, D, 4, 1><<<blocks_hashgrid, N_THREAD>>>(inputs, grad_grad_grid, offsets, grad_grad_out, B, L, S, H);
        break;
...
void kernel_grid_wrapper(const scalar_t *inputs, const scalar_t *embeddings, const int *offsets, scalar_t *outputs, const uint32_t B, const uint32_t C, const uint32_t L, const float S, const uint32_t H, const bool calc_grad_inputs, scalar_t *dy_dx) {
    ...
    switch (C) {
        case 1: kernel_grid<scalar_t, 3, 1><<<blocks_hashgrid, N_THREAD>>>(B, offsets, H, S, embeddings, inputs, outputs); 
		break;
        case 2: kernel_grid<scalar_t, 3, 2><<<blocks_hashgrid, N_THREAD>>>(B, offsets, H, S, embeddings, inputs, outputs); 
		break;
        case 4: kernel_grid<scalar_t, 3, 4><<<blocks_hashgrid, N_THREAD>>>(B, offsets, H, S, embeddings, inputs, outputs); 
        break;
        default: throw std::runtime_error{"GridEncoding: C must be 1, 2, 4, or 8."};
    }
}
...
void kernel_grid_backward_wrapper(const scalar_t *grad, const scalar_t *inputs, const scalar_t *embeddings, const int *offsets, scalar_t *grad_embeddings, const uint32_t B, const uint32_t C, const uint32_t L, const float S, const uint32_t H, const bool calc_grad_inputs, scalar_t *dy_dx, scalar_t *grad_inputs) {
    ...
    switch (C) {
        case 1:
        kernel_grid_backward<scalar_t, D, 1, 1><<<blocks_hashgrid, N_THREAD>>>(B, offsets, H, S, grad_embeddings, inputs, grad);
        if (calc_grad_inputs) 
            {
                kernel_grid_grad_inputs_pre<scalar_t, D, 1><<<blocks_hashgrid1, N_THREAD1>>>(B, offsets, H, S, embeddings, inputs, dy_dx);
                kernel_grid_backward_input<scalar_t, D><<<div_round_up(B * D, N_THREAD), N_THREAD>>>(B, L*C, grad, dy_dx, grad_inputs);
            }
        break;
        case 2: 
            kernel_grid_backward<scalar_t, D, 2, 1><<<blocks_hashgrid, N_THREAD>>>(B, offsets, H, S, grad_embeddings, inputs, grad);
            if (calc_grad_inputs) 
                {
                    kernel_grid_grad_inputs_pre<scalar_t, D, 2><<<blocks_hashgrid1, N_THREAD1>>>(B, offsets, H, S, embeddings, inputs, dy_dx);
                    kernel_grid_backward_input<scalar_t, D><<<div_round_up(B * D, N_THREAD), N_THREAD>>>(B, L*C, grad, dy_dx, grad_inputs);
                }
            break;
        case 4: 
            kernel_grid_backward<scalar_t, D, 4, 1><<<blocks_hashgrid, N_THREAD>>>(B, offsets, H, S, grad_embeddings, inputs, grad);
            if (calc_grad_inputs) 
                {
                    kernel_grid_grad_inputs_pre<scalar_t, D, 4><<<blocks_hashgrid1, N_THREAD1>>>(B, offsets, H, S, embeddings, inputs, dy_dx);
                    kernel_grid_backward_input<scalar_t, D><<<div_round_up(B * D, N_THREAD), N_THREAD>>>(B, L*C, grad, dy_dx, grad_inputs);
                }
            break;
